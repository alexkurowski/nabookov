defmodule App.Web.User do
  ...
  def maybe_signin(%{"email" => email, "token" => token}) do
    user = App.Repo.get_by(App.Web.User, email: email, signup_token: token)
    if is_nil(user) do
      App.Repo.get_by(App.Web.User, email: email)
  ...
  def fetch_by_email(email) do
    App.Repo.get_by(App.Web.User, email: email) ||
    App.Repo.insert(%App.Web.User{email: email})
    |> elem(1)
  end
  ...
  defp create_token(nil, _), do: %App.Web.User{}
